...
function Ftr_Paging_modified($tbl_name,$limit,$path,$cond="",$sql="")
{
	if($sql=="")
	{
		if($cond!=""){
...
{   
	$pagination .= "<div class='pagination'>";
if ($part > 1)
	$pagination.= "<a href='".$path."part=$prev' class='prev'>Previous</a>";
else
	$pagination.= "<span class='disabled'>Previous</span>";   
...
if ($part < $counter - 1)
	$pagination.= "<a href='".$path."part=$next' class='next'>Next</a>";
else
	$pagination.= "<span class='disabled'>Next</span>";
	$pagination.= "</div>\n";       
}
