...
class company {
	...
	public function save(){
		...
		if($result == 1)
		{
			...
			$this->flag = 2;		// fail
			return message::setMsg("Company creation failed","0");
		}
	}
	...
	// get posted data
		...
		if($result == 1)
		{
			...
			$this->flag = 2;		// fail
			return message::setMsg("Company updation failed","0");
		}
		...
		foreach($idvals as $idval){
			...
			mysql_query("UPDATE me_employee SET emp_status='2' WHERE company_id='$idval'");
			$result = dbFactory::recordDelete($idval,$tables);
			//mysql_query("DELETE FROM ".COMPANYVENDORTBL." WHERE company_id = '$idval'");
			if($result == 1)
			$count++;
		}
		if($count > 0){
			$this->flag = 1;		// success
			return message::setMsg("company(s) Successfully Deleted");
		}
	}
    ...
    public function disabled($idvals, $table, $column, $data = array())
	{
		 ...
		 foreach($idvals as $idval)
		 {
		   $sql=mysql_fetch_object(mysql_query("SELECT * FROM me_company WHERE id=$idval"));
			if($sql->c_status!=2)
			{
