  ...
  public function todaysTotalOrderPrice()
  {
	...
	public function mark_as_delivered()
	{
	    ...
	    if($upd)
		{
			...
			// insert into the billing table to show data in billing page...
			 $sql_bil = dbFactory::fetchAllData(TODAYSORDERS,array('vendor_id' => $_SESSION['vid']));
			if($sql_bil != '2') // i.e true
			{
			  while($asd = mysql_fetch_assoc($sql_bil))
			  {
			    mysql_query("INSERT INTO ".BILLING." (order_id,vendor_id,is_large_order,order_name,billing_date,buyer,company,phone,price,main_item,side_items) VALUES('".$asd['order_id']."','".$_SESSION['vid']."','".$asd['is_large_order']."','".mysql_real_escape_string($asd['order_name'])."',CURDATE(),'".mysql_real_escape_string($asd['buyer'])."','".mysql_real_escape_string($asd['company'])."','".$asd['phone']."','".$asd['price']."','".mysql_real_escape_string($asd['main_item'])."','".mysql_real_escape_string($asd['side_items'])."')");
				//echo "INSERT INTO ".BILLING." (order_id,vendor_id,is_large_order,order_name,billing_date,buyer,company,phone,price,main_item,side_items) VALUES('".$asd['order_id']."','".$_SESSION['vid']."','".$asd['is_large_order']."','".mysql_real_escape_string($asd['order_name'])."',CURDATE(),'".mysql_real_escape_string($asd['buyer'])."','".mysql_real_escape_string($asd['company'])."','".$asd['phone']."','".$asd['price']."','".mysql_real_escape_string($asd['main_item'])."','".mysql_real_escape_string($asd['side_items'])."')";
