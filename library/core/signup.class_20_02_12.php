...
class Signup {
	...
	public function signup_1(){
		...
		if($this->dom_exists($dom_ext['1']) == '1')
		{
		   ...
		   $result = dbFactory::recordInsert($datas,EMPLOYEETBL,'emp_email',array('unique_gen_code'=>$acode,'	emp_uname' => $uname));
		   if($result == '1')
		   {
	...
	public function signup_2($cond = array())
	{
		...
		unset($datas['emp_pwd']);
		   // update to db
		   $result = dbFactory::recordUpdate($cond,$datas,EMPLOYEETBL,array('emp_pwd' => $password,'create_date'=>date('Y-m-d h:i:s')));
		   if($result == '1')
		   {
