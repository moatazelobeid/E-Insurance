...
/*gets side items for a vendor form an order*/	
 ...
 /* public function todaysTotalOrderPrice()
  ...
  public function todaysTotalOrderPrice()
  {
  	$sel = dbFactory::fetchItem(array('vendor_id'=>''.$_SESSION['vid'].'','billing_date' => date('Y-m-d')),BILLING);
	if(isset($sel))
	{
	...
	public function mark_as_delivered()
	{
		...
		if(isset($sel))
		{
			...
			else
		    {
				...
				if($upd)
				{
					...
					if(isset($data))
					{
						$mail = mailFactory::itemDelivered($data['email'],$from = "admin@ollyspages.com",$res['order_id'],$data['name'],$data['t_price']);
					//	$mail = mailFactory::itemDeliveredAdmin($data['email'],$from = "info@maasinfotech.com",$res['order_id'],$data['name'],$data['t_price']);
				 	}
				}
			}
		...
		{   
		    ...
		    $mail = mailFactory::itemDeliveredAdmin($_SESSION['vid'],$from = "admin@ollyspages.com");
			return 1;
		}
		else
		{
	...
	public function check_if_delivered($orderId)
	{
		...
		else
		{
			...
			//	print_r($data);
				if(is_array($data))
				{
					$mail = mailFactory::itemDelivered($data['email'],$from = "admin@ollyspages.com",$orderId,$data['name'],$data['t_price']);
					//$mail = mailFactory::itemDeliveredAdmin($data['email'],$from = "admin@ollyspages.com",$orderId,$data['name'],$data['t_price']);
				}
				return 1;	
		   }
		}
...
public function getTodaysOrdersBilling()
{
	    $chkd = 0;
		$odr = dbFactory::fetchItem(array('vid'=>''.$_SESSION['vid'].''),'vendor_todays_order');
        $chk123 = dbFactory::fetchItem(array('vendor_id'=>''.$_SESSION['vid'].'','billing_date'=>date('Y-m-d'),'is_delivered'=>'0'),BILLING);
		if(isset($chk123))
		{
		...
		if($chkd > 0)
		{
			$chk = dbFactory::fetchItem(array('vendor_id'=>''.$_SESSION['vid'].'','billing_date'=>date('Y-m-d'),'is_delivered'=>'0'),BILLING);
			if(isset($chk))
			{
				$del = mysql_query("delete from ".BILLING." where vendor_id =  '".$_SESSION['vid']."' and billing_date = '".date('Y-m-d')."' and is_delivered ='0'");
			}
		}
		if(isset($odr))
		{
				...
				if(isset($largeOrder)) // if it is a large order ..
				{
			       ...
			       if(mysql_num_rows($sqlarge) > 0)
			       {
			        while($arr3 = mysql_fetch_assoc($sqlarge))
					 ...
					 $s_chk = mysql_query("select * from ".BILLING." where order_id = '".$rs['order_id']."'");   
					 if(mysql_num_rows($s_chk) == '0')
					 {
					$ins = dbFactory::recordInsert(array('order_id'=>$rs['order_id'],'vendor_id'=>$_SESSION['vid'],'is_large_order'=>'1', 'order_name'=>$order_name,'billing_date' => date('Y-m-d'),'buyer'=>$emp_name,'company'=>$emp_company,'phone'=>$eph,'price'=>$large_order_price,'main_item'=>'0','side_items'=>'','mealie_commission'=>$mealie_commision,'mealie_fees'=>$noca_charges,'is_delivered'=>'0'),BILLING);
					}
				 ...
				 $item_ss = get_item_name($q['item_id']);
				 $item_name = mysql_real_escape_string($item_ss['item_name']);
				 $s_chk2 = mysql_query("select * from ".BILLING." where order_id = '".$rs['order_id']."'");   
				 if(mysql_num_rows($s_chk2) == '0')
				 {
					$ins = dbFactory::recordInsert(array('order_id'=>$rs['order_id'],'vendor_id'=>$_SESSION['vid'],'is_large_order'=>'0', 'order_name'=>$order_name,'billing_date' => date('Y-m-d'),'buyer'=>$emp_name,'company'=>$emp_company,'phone'=>$eph,'price'=>$q['total_price'],'main_item'=>$q['item_id'],'side_items'=>mysql_real_escape_string($sideItems['name']),'mealie_commission'=>$mealie_commision,'mealie_fees'=>$noca_charges,'is_delivered'=>'0'),BILLING);
				  }
