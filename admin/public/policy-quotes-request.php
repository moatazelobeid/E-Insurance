	...
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),POLICYQUOTES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=policy-quotes-request';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=policy-quotes-request';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=policy-quotes-request';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),POLICYQUOTES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=policy-quotes-request';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=policy-quotes-request';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=policy-quotes-request';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),POLICYQUOTES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=policy-quotes-request';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=policy-quotes-request';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=policy-quotes-request';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),POLICYQUOTES) == 1){
...
<form action="" method="post" name="subject_fr" style="padding: 0px; margin: 0px;">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center"> 
    <tr>
       ...
       <?php  $totalsql=mysql_query("SELECT * FROM ".POLICYQUOTES." "); ?>
	   ...
	   <select name="policy_id" onchange="select_policy(this.value);" style="width: 150px; font-weight: normal; padding: 3px 3px 3px 3px;">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="3">
      <tr style="color: #FFF; background-color: #8B8585;">
	   ...
	   if(isset($_POST['stats'])){
		  ...
		  $rs=  mysql_query("SELECT * FROM ".POLICYQUOTES.$where." ORDER BY id DESC LIMIT ".$startpoint.",".$perpage);
		
	   }else{
		   $rs=mysql_query("SELECT * FROM ".POLICYQUOTES." ORDER BY id DESC LIMIT ".$startpoint.",".$perpage);
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
		...
		echo stripslashes($policy_class_title).' ('.stripslashes($insurance_types["policy_type"]).')'; ?></td>
		...
		<a href="public/policy-qoute-view.php?id=<?php echo $row['id']; ?>" id="fancy">
		<img src="images/view.png"  width="16" height="16" border="0" title="View Qoute Details" style="cursor: pointer;" /></a>&nbsp;
		<a href="account.php?page=policy-quotes-request&task=delete&id=<?php echo $row["id"];  ?> " onClick="return confirm('Are You Sure To Delete!');">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 5px;">
  <tr>
    ...
    <td width="19%">&nbsp;</td>
    <td width="75%" align="right"><?php echo Paging(POLICYQUOTES,$perpage,"account.php?page=policy-quotes-request".$sub_url."&",$pwhr);?></td>
  </tr>
</table>
<!-- @end users list -->
</form>