...
if(isset($_POST['save']))
{
	$latest_qt = mysql_fetch_object(mysql_query("SELECT AUTO_INCREMENT FROM information_schema.tables WHERE TABLE_NAME = '".TRAVEL_PACKAGE."'"));
	...
	$result = $db->recordInsert($PST,TRAVEL_PACKAGE,'');
...
if(isset($_POST['update']))
{
	...
	$result = $db->recordUpdate(array('id'=>$_GET['id']),$PST,TRAVEL_PACKAGE,'');
	
	// Get Package No.//
	$package_no = array_shift(mysql_fetch_array(mysql_query("SELECT package_no FROM ".TRAVEL_PACKAGE." WHERE id = '".$_GET['id']."'")));
...
{
  $sql_edit = mysql_fetch_object(mysql_query("SELECT * FROM ".TRAVEL_PACKAGE." WHERE id = '".$_GET['id']."'"));
