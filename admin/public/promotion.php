	...
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
					$db->recordDelete(array("id" => $id),PROMOTION);
					$count++;
				
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=promotion&view=list';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=promotion&view=list';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=promotion&view=list';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),PROMOTION) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=promotion&view=list';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=promotion&view=list';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=promotion&view=list';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),PROMOTION) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Unpublished successfully');location.href='account.php?page=promotion&view=list';</script>";
			}else{
				echo "<script>alert('No Records Unpublished');location.href='account.php?page=promotion&view=list';</script>";
			}
		}else{
			echo "<script>alert('No Records Unpublished');location.href='account.php?page=promotion&view=list';</script>";
		}
...
if($_REQUEST['task']=='delete')
{
 ...
 $sqql="delete from ".PROMOTION." where id='".$_REQUEST['id']."'";
...
if($_GET['task']=='block'){
	...
	$sqlu=mysql_query("update ".PROMOTION." set status='0' where id='$id'");
if(mysql_affected_rows() > 0)
echo "<script>alert('Record Unpublished Sucessfully');location.href='account.php?page=promotion&view=list&part=".$_GET['part']."';</script>";
else
echo "<script>alert('Record Unpublished Failed');location.href='account.php?page=promotion&view=list&part=".$_GET['part']."';</script>";
...
if($_GET['task']=='active'){
	...
	$sqlp="update ".PROMOTION." set status='1' where id='$id'";

	$resultp=mysql_query($sqlp);
if(mysql_affected_rows() > 0)
echo "<script>alert('Record Published Sucessfully');location.href='account.php?page=promotion&view=list&part=".$_GET['part']."';</script>";
else
echo "<script>alert('Record Published Failed');location.href='account.php?page=promotion&view=list&part=".$_GET['part']."';</script>";
...

{
	...
	$btn_value = " Add New Promotion ";
	$cancel = "<a href='account.php?page=promotion&view=list'><u>Cancel</u></a>";
}
...

{
	...
	$cancel = "<a href='account.php?page=promotion&view=list'>Cancel</a>";
...

{
	...
	$sq_save = "INSERT INTO ".PROMOTION." (`title`, `title_ar`, `desc`, `desc_ar`, `status`, `created_date`, `modified_date`) VALUES ('".$title."','".$title_ar."','".$desc."','".$desc_ar."','".$status."',now(),'0000-00-00 00:00:00')";

//echo $sq_save; exit;
	$res_save = mysql_query($sq_save);
    header("location:account.php?page=promotion&view=list");
}
...

{
	...
	$sq_var = "SELECT * FROM ".PROMOTION." WHERE id = '".$_GET['id']."'";
...

{
	...
	$sq_update = "UPDATE ".PROMOTION." SET `title` = '".$title."', `title_ar` = '".$title_ar."', `desc` = '".$desc."', `desc_ar` = '".$desc_ar."', `status` = '".$status."' WHERE id = ".$page_id;	
...
<div style="width: 800px; margin: 0 auto; margin-top: 10px;">
  <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
    <tr>
      <td width="67%" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Add/Edit Promotion</strong></td>
      <td width="33%" align="right" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-right: 0px; font-size: 14px; color: #036;"><a href="account.php?page=promotion&view=list"><img src="images/view_all.png" width="87" height="15" border="0"></a></td>
    </tr>
    <?php if($msg <> ""){
?>
...
<form action="" method="post" name="partcat_form" id="partcat_form" onSubmit="return masterValidate()">  <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
    <tr>
      <td colspan="2" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>List All Promotions</strong></td>
      <td width="33%" align="right" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-right: 0px; font-size: 14px; color: #036;"><a href="account.php?page=promotion"><img src="images/add_new.png" width="87" height="15" border="0"></a></td>
    </tr>
    <tr>
      <td width="29%" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><input type="button" name="deleteall" id="deleteall" value="Delete " style="font-family: Verdana, Geneva, sans-serif; font-size: 11px;" onclick="document.partcat_form.todo.value='deleteall';document.partcat_form.submit();" disabled="disabled"/>
...
if(isset($_POST["search"])){
		  $j = 0;
		  $i = 0;
		 $sq = "SELECT * FROM ".PROMOTION." WHERE title LIKE '%".$_POST["stitle"]."%' ORDER BY id ASC LIMIT ".$startpoint.",".$perpage;
	  }	else
	  {			
				
		  $sq = "select * from ".PROMOTION." ORDER BY id LIMIT ".$startpoint.",".$perpage;}
          ...
          <tr <?php echo $bgcolor; ?>>
			...
			if($row["status"]=='0') { ?>
              <a href="account.php?page=promotion&task=active&view=list&id=<?php echo $row["id"]; ?>"><font color="#FF0000">InActive</font>
              <?php } else {?>
              </a><a href="account.php?page=promotion&task=block&view=list&id=<?php echo $row["id"]; ?>"><font color="#006633">Active</font>
                  
                  </a>
            <?php } ?></td>
			            <td align="center" ><?php echo date("d/m/Y",strtotime($row["created_date"])); ?></td>

            <td align="center" ><a href="public/promotion_view.php?id=<?php echo $row['id']; ?>" id="fancy"><img src="images/view.png"  width="16" height="16" border="0" title="View" style="cursor: pointer;" /></a>&nbsp;<a href="account.php?page=promotion&task=edit&id=<?php echo $row["id"];  ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=promotion&task=delete&view=list&id=<?php echo $row["id"];  ?>" onclick="return confirm('ARE YOU SURE TO DELETE!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete" style="cursor: pointer;"></a></td>
          </tr>
          <?php $i=$i+1;} ?>
          <tr>
            <td colspan="8" align="left"><input type="button" name="deleteall" id="deleteall1" value="Delete " style="font-family: Verdana, Geneva, sans-serif; font-size: 11px;" onclick="document.partcat_form.todo.value='deleteall';document.partcat_form.submit();" disabled="disabled"/>
      ...
      <input type="hidden" name="todo" id="todo" value=""/></td>
          ...
          <tr>
            <td colspan="8" align="center"><?php
			 ...
			 if(isset($_POST['search'])){
				  echo Paging(PROMOTION,$perpage,"account.php?page=promotion&view=list&","title LIKE '%".$_POST['stitle']."%' ORDER BY id DESC");
					
					}else{
					echo Paging(PROMOTION,$perpage,"account.php?page=promotion&view=list&");
					} 
