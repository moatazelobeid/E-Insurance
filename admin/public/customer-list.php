	...
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id'=>$id),'ksa_user') == 1){
					$db->recordDelete(array('uid'=>$id,'user_type'=>'U'),LOGINTBL);
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=customer-list';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=customer-list';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=customer-list';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=customer-list';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=customer-list';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=customer-list';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			if($count > 0){
				echo "<script>alert('Records Unpublished successfully');location.href='account.php?page=customer-list';</script>";
			}else{
				echo "<script>alert('No Records Unpublished');location.href='account.php?page=customer-list';</script>";
			}
		}else{
			echo "<script>alert('No Records Unpublished');location.href='account.php?page=customer-list';</script>";
		}
...
{
	if($db->recordDelete(array('id'=>$_GET['id']),'ksa_user') == 1){
		$db->recordDelete(array('uid'=>$_GET['id'],'user_type'=>'U'),LOGINTBL);
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 4px; margin-top: 10px;">
    <tr>
      ...
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>List/Manage Customers</strong></td>
      <td align="right" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;">

<a href="account.php?page=add-customers"><img src="images/add_new.png" width="87" height="15" border="0"></a>
    ...
    <tr>
       ...
       <td width="62%"  align="right" style="border-bottom: 0px ; padding-bottom: 0px; padding-left: 1px; padding-right: 1px; font-size: 14px; color: #036;">
         <input name="sertxt" type="text" class="textbox" id="sertxt"  style="width: 150px; font-weight: normal;" value="<?php echo $_POST['sertxt']; ?>">
            <select name="search_by_source" style=" width: 150px; " id="search_by_source"><option value="">Customer Source</option><option value="Online" <?php if($_POST["search_by_source"] == "Online") echo "selected='selected'"; ?>>Online</option><option value="Offline" <?php if($_POST["search_by_status"] == "Offline") echo "selected='selected'"; ?>>Offline</option></select>
        <select name="search_by_status" style=" width: 150px; " id="search_by_status"><option value="">Status</option><option value="1" <?php if($_POST["search_by_status"] == "1") echo "selected='selected'"; ?>>Active</option><option value="0" <?php if($_POST["search_by_status"] == "0") echo "selected='selected'"; ?>>Inactive</option></select>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 2px;">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="2%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll"  onclick="check_all();" /></td>
      <td width="7%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Cus.Code</strong></td>
       <td width="16%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Name</strong></td>
     <td width="14%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Email Id</strong></td>
     <td width="13%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Phone (M)</strong></td>
      <td width="11%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>No of policies</strong></td>
       ...
       <?php 
		...
		{
			if($_POST['search_by_status']!="" && $_POST['sertxt'] !="")
			{
				$sq = "SELECT ksa_user.*,".LOGINTBL.".uname,".LOGINTBL.".pwd,".LOGINTBL.".is_active FROM ksa_user ,".LOGINTBL." where ksa_user.id = ".LOGINTBL.".uid 
						   AND ".LOGINTBL.".user_type = 'U' AND (".LOGINTBL.".is_active LIKE '".$_POST['search_by_status']."') AND (ksa_user.fname LIKE '%".$_POST['sertxt']."%' OR ksa_user.lname LIKE '%".$_POST['sertxt']."%' OR ksa_user.customer_code	 LIKE '%".$_POST['sertxt']."%'
							OR ksa_user.email LIKE '%".$_POST['sertxt']."%' OR ".LOGINTBL.".uname LIKE '%".$_POST['sertxt']."%') LIMIT ".$startpoint.",".$perpage;
			} 
			elseif($_POST['search_by_source']!="") 
			{
				$sq = "SELECT ksa_user.*,".LOGINTBL.".uname,".LOGINTBL.".pwd,".LOGINTBL.".is_active FROM ksa_user,".LOGINTBL."  where ksa_user.id = ".LOGINTBL.".uid 
					   AND ".LOGINTBL.".user_type = 'U' AND ksa_user.customer_source LIKE '%".$_POST['search_by_source']."%' LIMIT ".$startpoint.",".$perpage; 
			}
			elseif($_POST['search_by_status']!="") 
			{
				$sq = "SELECT ksa_user.*,".LOGINTBL.".uname,".LOGINTBL.".pwd,".LOGINTBL.".is_active FROM ksa_user,".LOGINTBL."  where ksa_user.id = ".LOGINTBL.".uid 
					   AND ".LOGINTBL.".user_type = 'U' AND (".LOGINTBL.".is_active LIKE '".$_POST['search_by_status']."') LIMIT ".$startpoint.",".$perpage; 
			}
			else 
			{
				$sq = "SELECT ksa_user.*,".LOGINTBL.".uname,".LOGINTBL.".pwd,".LOGINTBL.".is_active FROM ksa_user ,".LOGINTBL." where ksa_user.id = ".LOGINTBL.".uid 
						   AND ".LOGINTBL.".user_type = 'U'  AND (ksa_user.fname LIKE '%".$_POST['sertxt']."%' OR ksa_user.lname LIKE '%".$_POST['sertxt']."%' OR ksa_user.customer_code	 LIKE '%".$_POST['sertxt']."%'
							OR ksa_user.email LIKE '%".$_POST['sertxt']."%' OR ".LOGINTBL.".uname LIKE '%".$_POST['sertxt']."%') LIMIT ".$startpoint.",".$perpage; 
		...
		{
			$sq = "SELECT c1.*,c2.uname,c2.pwd,c2.is_active FROM ksa_user AS c1,".LOGINTBL." AS c2 where c1.id = c2.uid 
				   AND c2.user_type = 'U' LIMIT ".$startpoint.",".$perpage;
		}
		//echo $sq;
		//echo $_SESSION['aid'];
		$j=0;
		...
		$l_info = mysql_fetch_array(mysql_query("select * from ".LOGINTBL." where uid = '".$row['id']."' AND user_type = 'U'"));
    ...
    <input type="checkbox" name="chkNo[]" id="chkNo<?php echo $i;?>" value="<?php echo $row['id']; ?>" onclick="check_single('chkNo<?php echo $i;?>');" <?php if($_SESSION['aid']==$row["id"]){?> disabled="disabled"<?php }?>/>
         ...
         <td align="center" style="padding-left: 5px;"><?php echo $row["customer_code"]; ?></td>
		<td align="left" style="padding-left: 5px;"><?php echo stripslashes($row["salutation"])." ".stripslashes($row["fname"])." ".stripslashes($row["lname"]); ?></td>
	    <td align="left" style="padding-left: 5px;"><?php echo $row["email"]; ?></td>
         <td align="left" style="padding-left: 5px;"><?php echo $row["phone_mobile"]; ?></td>
         <td align="center" style="padding-left: 5px;"><?php $policies = mysql_query("select * from ".POLICYMASTER." where customer_id = '".$row['customer_code']."'");
		 	echo mysql_num_rows($policies);
		  ?></td>
		<td width="13%" align="center" >
		
       		<?php if($l_info["is_active"]=='0') { ?>
        	<a href="account.php?page=customer-list&task=activate&id=<?php echo $row["id"]; ?>"><span style="color:#FF0000">In Active</span> </a>
 			<?php } else {  ?>
  			<a href="account.php?page=customer-list&task=block&id=<?php echo $row["id"]; ?>"><span style="color:#390">Active</span> </a>
			<?php } ?>
        
        </td>
		<td width="13%" align="center" ><?php echo date("d/m/Y",strtotime($row["created_date"])); ?></td>
        <td width="11%" align="center" >
        
        <a  href="account.php?page=view_customer_details&id=<?php echo $row["id"]; ?>">
		...
		<a href="account.php?page=add-customers&amp;task=edit&amp;id=<?php echo $row["id"];  ?>">
        ...
        <a href="account.php?page=customer-list&amp;task=delete&amp;id=<?php echo $row["id"];  ?> " onclick="return confirm('Are You Sure To Delete!');">
  	...
  	<?php $i++;}}else{ ?>
  <tr>
  	<td colspan="8" align="center" bgcolor="#F2FBFF">No Customers Listed</td>
  </tr>
  <?php  }  ?>
 </table></td>
  </tr>
    ...
    <td align="center" valign="top" style="padding-top: 10px;">
		<?php
			if(isset($_POST['search']))
			{
				if($_POST['search_by_status']!="" && $_POST['sertxt'] !="")
				{
					echo Paging('ksa_user,ksa_login',$perpage,"account.php?page=customer-list&","ksa_user.id = ".LOGINTBL.".uid 
						   AND ".LOGINTBL.".user_type = 'U' AND (".LOGINTBL.".is_active LIKE '".$_POST['search_by_status']."') AND (ksa_user.fname LIKE '%".$_POST['sertxt']."%' OR ksa_user.lname LIKE '%".$_POST['sertxt']."%' OR ksa_user.customer_code	 LIKE '%".$_POST['sertxt']."%'
							OR ksa_user.email LIKE '%".$_POST['sertxt']."%' OR ".LOGINTBL.".uname LIKE '%".$_POST['sertxt']."%')");
				}
				elseif($_POST['search_by_source']!="") 
				{
					echo Paging('ksa_user,ksa_login',$perpage,"account.php?page=customer-list&","ksa_user.id = ".LOGINTBL.".uid 
						   AND ".LOGINTBL.".user_type = 'U' AND ksa_user.customer_source LIKE '%".$_POST['search_by_source']."%'");
					
				}
				elseif($_POST['search_by_status']!="")
				{
					echo Paging('ksa_user,ksa_login',$perpage,"account.php?page=customer-list&","ksa_user.id = ".LOGINTBL.".uid 
					   AND ".LOGINTBL.".user_type = 'U' AND (".LOGINTBL.".is_active LIKE '".$_POST['search_by_status']."')");
				}
				else 
				{
					echo Paging('ksa_user,ksa_login',$perpage,"account.php?page=customer-list&","ksa_user.id = ".LOGINTBL.".uid 
			...
			{
            	echo Paging('ksa_user',$perpage,"account.php?page=customer-list&","id IN (SELECT uid FROM ".LOGINTBL." )");
			}
        ?>
    </td>
  </tr>
  <tr>
    <td align="left" valign="top" style="padding-top: 10px;"><span style="color:#FF0000"><b>Note: </b></span>Search can be posible by Customer code, First Name, Last Name or Email Id or Status . </td>
  </tr>
</table>
</div>