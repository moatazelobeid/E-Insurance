...
if(isset($_POST['save']))
{
	...
	if($db->isExists('office_name',$_POST,OFFICES)){
		$errmsg = "- Office Name already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,OFFICES,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Office Created Sucessfully');location.href='account.php?page=offices';</script>";
	else if($result == 2)
		echo "<script>alert('Office Creation Failed');location.href='account.php?page=offices';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,OFFICES) == 1){
	echo "<script>alert('Office Updated Sucessfully');location.href='account.php?page=offices';</script>";
	}else
	echo "<script>alert('Office Updation Failed');location.href='account.php?page=offices&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,OFFICES.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),OFFICES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=offices';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=offices';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=offices';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),OFFICES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=offices';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=offices';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=offices';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),OFFICES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=offices';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=offices';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=offices';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),OFFICES) == 1){
...
function validateManager()
{
	...
	if(!nameRegex.test(str.office_name.value.trim()))
	{
		str.office_name.style.borderColor = "RED";
		error = "- Enter Office Name\n";
		flag = false;
		dataArray.push('office_name');
	}
	else
	{
		str.office_name.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	if(flag == false)
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    ...
    <tr>
      ...
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Create/Manage Office </strong></td>
...
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="108%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Office Name(EN):</td>
        <td style="padding-right: 0px;"><input name="office_name" type="text" class="textbox" id="office_name" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('office_name',$datalist); ?>"></td>
      </tr>
	  <tr>
        <td style="padding-left: 0px;">Office Name(AR):</td>
        <td style="padding-right: 0px;"><input name="office_name_ar" type="text" class="textbox" id="office_name_ar" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('office_name_ar',$datalist); ?>"></td>
      </tr>
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onClick="location.href='account.php?page=offices'">        </td>
        </tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="6%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onClick="check_all();" /></td>
       ...
       <td width="48%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Office Name </strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".OFFICES." WHERE office_name LIKE '%".$_POST["sertxt"]."%' ORDER BY id ASC"); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".OFFICES." ORDER BY id ASC");
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
		...
		<td align="left"  style="padding-left: 5px;"><?php echo stripslashes($row["office_name"]); ?></td>
		<td width="19%" align="center" >
		  <?php if($row['status'] == 0) echo "Inactive"; else echo "Active"; ?></td>
        <td width="19%" align="center" ><a href="account.php?page=offices&task=edit&id=<?php echo $row["id"]; ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit User Profile" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=offices&task=delete&id=<?php echo $row["id"];  ?> " onClick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;"></a></td>
  </tr>
        <?php }}else{
			?>
            <tr>
