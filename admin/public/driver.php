...
if(isset($_POST['save']))
{
	...
	if($db->isExists('age',$_POST,DRIVERAGE)){
		$errmsg = "- Driver Age already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,DRIVERAGE,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Driver Age Created Sucessfully');location.href='account.php?page=driver';</script>";
	else if($result == 2)
		echo "<script>alert('Driver Age Creation Failed');location.href='account.php?page=driver';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,DRIVERAGE) == 1){
	echo "<script>alert('Driver Age Updated Sucessfully');location.href='account.php?page=driver';</script>";
	}else
	echo "<script>alert('Driver Age Updation Failed');location.href='account.php?page=driver&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,DRIVERAGE.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),DRIVERAGE) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=driver';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=driver';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=driver';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),DRIVERAGE) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=driver';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=driver';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=driver';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),DRIVERAGE) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=driver';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=driver';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=driver';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),DRIVERAGE) == 1){
...
function validateManager()
{
	...
	if(str.age.value == "")
	{
		str.age.style.borderColor = "RED";
		error = "- Enter Driver Age\n";
		flag = false;
		dataArray.push('age');
	}
	else
	{
		str.age.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	if(str.driver.value == "")
	{
		str.driver.style.borderColor = "RED";
		error = "- Enter Driver Age\n";
		flag = false;
		dataArray.push('driver');
	}
	else
	{
		str.driver.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	if(flag == false)
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    ...
    <tr>
      ...
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Create/Manage Driver Age </strong></td>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
          <tr>
            <td align="left" style="border-bottom: 0px solid #99C; padding: 3px; background-color: #f2f2f2;"><strong>Add/Edit Driver Age</strong></td>
          </tr>
        </table>
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="108%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Driver Age(EN) :</td>
        <td style="padding-right: 0px;"><input name="age" type="text" class="textbox" id="age" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('age',$datalist); ?>"></td>
      </tr>
	  <tr>
        <td style="padding-left: 0px;">Driver Age(AR) :</td>
        <td style="padding-right: 0px;"><input name="age_ar" type="text" class="textbox" id="driver" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('age_ar',$datalist); ?>"></td>
      </tr>
      ...
      <tr>
        ...
        <td width="74%" style="padding-right: 0px;"><select name="status" id="status" style="width: 193px; font-weight: normal;">
          ...
          <option value="0" <?php if(getElementVal('status',$datalist) == "") {echo "selected='selected'";} ?>>Inactive</option>
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onclick="location.href='account.php?page=driver'">        </td>
        </tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="6%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onclick="check_all();" /></td>
       <td width="56%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Driver Age </strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".DRIVERAGE." WHERE age LIKE '%".$_POST["sertxt"]."%' ORDER BY id DESC"); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".DRIVERAGE." ORDER BY id DESC");
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
		...
		<td align="left"  style="padding-left: 5px;"><?php echo stripslashes($row["age"]); ?></td>
        
		<td width="19%" align="center" >
		  <?php if($row['status'] == "Active") {echo "Active";} else {echo "Inactive";} ?></td>
        <td width="19%" align="center" ><a href="account.php?page=driver&task=edit&id=<?php echo $row["id"];  ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit User Profile" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=driver&task=delete&id=<?php echo $row["id"];  ?> " onclick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;"></a></td>
  </tr>
        <?php }}else{
			?>
            <tr>
