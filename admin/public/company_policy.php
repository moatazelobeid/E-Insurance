<?php // $aa=array('aa', '', ''); echo '<pre>'; print_r($aa); echo '</pre>'; exit;
...
if($_GET['task']=='delete' && $_GET['id'])
{
	...
	{
		$table=HEALTHPOLICY;
		$datalists = $db->recordFetch($id,HEALTHPOLICY.":".'id');
		//$doc='../upload/health_policy/'.getElementVal('doc_url',$datalists);
		//unlink($doc);
	}
	if($_GET['type']=='malpractice')
		$table=MALPRACTICEPOLICY;
	
	$res_policy=mysql_fetch_object(mysql_query("SELECT * FROM ".$table." WHERE id='".$id."'"));
	$comp_pol_id=$res_policy->comp_policy_id;
	$db->recordDelete(array('comp_policy_id' => $comp_pol_id),OFFERS);
	
   if($db->recordUpdate(array("id" => $id),array('del_status' => '1'),$table) == 1){
   echo "<script>alert('Policy Deleted Sucessfully');location.href='account.php?page=company_policy';</script>";
		/*$msg='Policy deleted successfully.';
		header('Location: account.php?page=company_policy');*/
	}
}
...
if(isset($_POST['todo'])){
	...
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=company_policy';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=company_policy';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=company_policy';</script>";
		}
		break;
...
function searchByType(val)
{
	...
	if(val!='')
		typeurl='&type='+val;
	var url='<?php echo BASE_URL;?>account.php?page=company_policy'+typeurl+turl+sturl+compurl;
...
function checkUpdate(val,id,type)
{
$.ajax({
   			 type: "POST",
  			 url: "util/utils.php",
  			 data: "status="+val+"&id="+id+"&type="+type,
...
function confirmInput()
 {
  var retVal = confirm("Do you want to Delete these Company policy ?");
   if( retVal == true ){
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" style="margin-bottom: 5px;  line-height: 15px; color: #666;">
          <tr>
            <td width="34%" align="left" style="padding-left: 0px; font-size: 14px; color: #036;"><strong>List/Manage Company Policies </strong></td>
		...
		<table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" style="margin-bottom: 5px;  line-height: 15px; color: #666;">
          <tr>
            <td align="right" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-top:3px; padding-left: 0px; font-size: 14px; color: #036;">
					...
					<input type="button" name="reset" id="reset" value="Reset" class="actionBtn" onclick="location.href='account.php?page=company_policy'" style="font-family: Verdana, Geneva, sans-serif; font-size: 11px;" />            </td>
          </tr>
        </table>
                  ...
                  <tr>
                    <td style="border-top: 0px solid #99C; padding-left: 0px;"><table width="100%" border="0" cellspacing="1" cellpadding="2">
                        <tr style="color: #FFF;">
                          ...
                          <td width="10%" align="center" bgcolor="#006699" style="padding-left: 5px;"><strong>Policy ID# </strong></td>
                          <td width="35%" align="left" bgcolor="#006699"><strong>Title</strong></td>
                          <td width="16%" align="left" bgcolor="#006699"><strong>Company</strong></td>
                          <td width="12%" align="center" bgcolor="#006699"><strong>Policy Type</strong></td>
                          <td width="8%" align="center" bgcolor="#006699"><strong>Created On</strong></td>
                          <td width="9%" align="center" bgcolor="#006699"><strong>Status</strong></td>
                          <td width="7%" align="center" bgcolor="#006699"><strong>Action</strong></td>
                        </tr>
                        <?php 
		...
		if($i % 2 == 0){ $bgcolor = "#F2F3F5"; }else{ $bgcolor = "#FFFFFF";  }
			  ?>
                        <tr bgcolor="<?php echo $bgcolor;?>">
                          <td align="center">
						  <input type="checkbox" name="chkNo[]" id="chkNo<?php echo $i;?>" value="<?php echo $row['comp_policy_id']; ?>" onclick="check_single('chkNo<?php echo $i;?>');" /> 
						  <input type="hidden" name="pol_type<?php echo $row['comp_policy_id']; ?>" id="pol_type<?php echo $i;?>" value="<?php echo $row['policy_type'];?>" /></td>
                          <td align="center"  style="padding-left: 5px;"><?php echo $row["comp_policy_id"]; ?></td>
                          <td width="35%" align="left"><?php echo stripslashes($row["title"]); ?></td>
                          ...
                          <td width="12%" align="center"><?php echo ucwords($row['policy_type']); if($row['policy_type']=='auto') echo ' ('.getAutoPolicyType($row['id']).')';?></td>
                          
						  
                          <td width="8%" align="center" ><?php echo date('d-m-Y',strtotime($row["created_date"])); ?></td>
                          <?php if(isset($_GET['search'])){
			$url="account.php?page=company_policy&sertxt=".$_GET['sertxt']."&emp_type=".$_GET['emp_type']."&search=search";
			} else $url="account.php?page=company_list"; ?>
                          <td width="9%" align="center" >
						  <select id='status' name='status' onchange="checkUpdate(this.value,<?php echo $row["id"]; ?>,'<?php echo $row['policy_type']?>')" >
                          ...
                          <td width="7%" align="center" ><a id="box1_policy" href="public/company-policy-details.php?id=<?php echo $row["id"]; ?>&type=<?php echo $row["policy_type"]; ?>"><img src="images/b_browse.png" alt="View Company Details" width="16" height="16" border="0" title="View Company Details" style="cursor: pointer;" /></a>&nbsp;
						  
					<a href="account.php?page=company_policy&amp;task=delete&amp;id=<?php echo $row["id"];?>&amp;type=<?php echo $row['policy_type'];?>" onclick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;" /></a></td>
                        </tr>
                        <?php $i++;}}else{
			?>
                        <tr>
                          <td colspan="10" align="center" bgcolor="#F2FBFF">No Policy added by any Company </td>
                        </tr>
                        <?php
		}?>
                    </table></td>
  ...
  <tr>
							<td colspan="7" style="padding-top:10px" align="center">
								<?php echo Paging('',$perpage,"account.php?page=company_policy".$suburl."&",'',$sql_paging);?>			
							</td>
						</tr>
  <tr>
    <td align="left" valign="top" style="padding-top: 10px;"><span style="color:#FF0000"><b>Note: </b></span>Search can be posible by Company Name or Policy Type or Policy ID. </td>
  </tr>
</table>
