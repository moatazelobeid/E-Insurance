...
if(isset($_POST['save']))
{
	...
	if($db->isExists('policy_type',$_POST,POLICYTYPES)){
		$errmsg = "- Policy Type already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,POLICYTYPES,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Policy Type Created Sucessfully');location.href='account.php?page=policies&view=policy-type';</script>";
	else if($result == 2)
		echo "<script>alert('Policy Type Creation Failed');location.href='account.php?page=policies&view=policy-type';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,POLICYTYPES) == 1){
	echo "<script>alert('Policy Type Updated Sucessfully');location.href='account.php?page=policies&view=policy-type';</script>";
	}else
	echo "<script>alert('Policy Type Updation Failed');location.href='account.php?page=policies&view=policy-type&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,POLICYTYPES.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),POLICYTYPES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=policies&view=policy-type';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=policies&view=policy-type';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=policies&view=policy-type';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),POLICYTYPES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=policies&view=policy-type';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=policies&view=policy-type';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=policies&view=policy-type';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),POLICYTYPES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=policies&view=policy-type';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=policies&view=policy-type';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=policies&view=policy-type';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),POLICYTYPES) == 1){
...
function confirmInput()
 {
  var retVal = confirm("Do you want to Delete these Policy Type ?");
   if( retVal == true ){
...
function validateManager()
{
	...
	if(str.policy_id.value == "")
	{
		str.policy_id.style.borderColor = "RED";
		error = "- Select Policy Class\n";
		flag = false;
		dataArray.push('policy_id');
	}
	else
	{
		str.policy_id.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	
	if(!nameRegex.test(str.policy_type.value.trim()))
	{
		str.policy_type.style.borderColor = "RED";
		error += "- Enter Policy Type\n";
		flag = false;
		dataArray.push('policy_type');
	}
	else
	{
		str.policy_type.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	if(flag == false)
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    <tr>
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 10px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Create/Manage Policy Type </strong></td>
      <td align="right" style="border-bottom: 1px solid #CCC; padding-bottom: 15px; padding-right: 0px;">
	  <a href="account.php?page=policies" class="linkBtn">Manage Policy Class</a>
	  </td>
    </tr> 
</table>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
          <tr>
            <td align="left" style="border-bottom: 0px solid #99C; padding: 3px; background-color: #f2f2f2;"><strong>Add/Edit Policy Type </strong></td>
          </tr>
      </table>
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="108%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Policy Class: </td>
        <td style="padding-right: 0px;">
		<select name="policy_id" style="width: 193px; font-weight: normal;">
			<option value="">Select Class</option>
			<?php 
			$sql_class = mysql_query("SELECT id,title FROM ".POLICIES);
			if(mysql_num_rows($sql_class) > 0){
				while($result_class = mysql_fetch_array($sql_class))
				{
				?>
					<option value="<?php echo $result_class['id'];?>" <?php if(getElementVal('policy_id',$datalist)==$result_class['id'])echo 'selected="selected"';?>><?php echo $result_class['title'];?></option>
				<?php
				}
			}
			?>
      ...
      <tr>
        <td style="padding-left: 0px;">Policy Type(EN):</td>
        <td style="padding-right: 0px;"><input name="policy_type" type="text" class="textbox" id="policy_type" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('policy_type',$datalist); ?>"></td>
      </tr>
	  <tr>
        <td style="padding-left: 0px;">Policy Type(AR):</td>
        <td style="padding-right: 0px;"><input name="policy_type_ar" type="text" class="textbox" id="policy_type_ar" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('policy_type_ar',$datalist); ?>"></td>
      </tr>
	  
      <tr>
        
        <td width="26%" style="padding-left: 0px;">Status:</td>
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onClick="location.href='account.php?page=policies&view=policy-type'">        </td>
        </tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="6%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onClick="check_all();" /></td>
       ...
       <td width="31%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Policy Type </strong></td>
       <td width="26%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Policy Class </strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".POLICYTYPES." WHERE (policy_type LIKE '%".$_POST["sertxt"]."%' OR policy_type_ar LIKE '%".$_POST["sertxt"]."%') ORDER BY id ASC"); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".POLICYTYPES." ORDER BY id ASC");
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
		...
		<td align="left"  style="padding-left: 5px;"><?php echo stripslashes($row["policy_type"]); ?></td>
		<td align="left"  style="padding-left: 5px;"><?php $policy_class_title = $db->recordFetch($row["policy_id"],POLICIES.":".'id'); echo getElementVal('title',$policy_class_title); ?></td>
		<td width="15%" align="center">
		  <?php if($row['status'] == 0) echo "Inactive"; else echo "Active"; ?></td>
        <td width="14%" align="center" ><a href="account.php?page=policies&view=policy-type&task=edit&id=<?php echo $row["id"]; ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit User Profile" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=policies&view=policy-type&task=delete&id=<?php echo $row["id"];  ?> " onClick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;"></a></td>
  </tr>
        <?php }}else{
			?>
            <tr>
