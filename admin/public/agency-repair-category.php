...
if(isset($_POST['save']))
{
	...
	if($db->isExists('cat_name',$_POST,AGENCYREPAIR)){
		$errmsg = "- Category Name already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,AGENCYREPAIR,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Category Created Sucessfully');location.href='account.php?page=agency-repair-category';</script>";
	else if($result == 2)
		echo "<script>alert('Category Creation Failed');location.href='account.php?page=agency-repair-category';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,AGENCYREPAIR) == 1){
	echo "<script>alert('Category Updated Sucessfully');location.href='account.php?page=agency-repair-category';</script>";
	}else
	echo "<script>alert('Category Updation Failed');location.href='account.php?page=agency-repair-category&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,AGENCYREPAIR.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),AGENCYREPAIR) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=agency-repair-category';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=agency-repair-category';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=agency-repair-category';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),AGENCYREPAIR) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=agency-repair-category';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=agency-repair-category';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=agency-repair-category';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),AGENCYREPAIR) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=agency-repair-category';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=agency-repair-category';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=agency-repair-category';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),AGENCYREPAIR) == 1){
...
function validateManager()
{
	...
	if(str.cat_name.value == "")
	{
		str.cat_name.style.borderColor = "RED";
		error = "- Enter Category Name\n";
		flag = false;
		dataArray.push('cat_name');
	}
	else
	{
		str.cat_name.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	if(str.percentage.value == "")
	{
		str.percentage.style.borderColor = "RED";
		error = "- Enter Percentage\n";
		flag = false;
		dataArray.push('percentage');
	}
	else
	{
		str.percentage.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	if(flag == false)
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    ...
    <tr>
      ...
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Manage Agency Repair Category</strong></td>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
          <tr>
            <td align="left" style="border-bottom: 0px solid #99C; padding: 3px; background-color: #f2f2f2;"><strong>Add/Edit Agency Repair Category</strong></td>
          </tr>
        </table>
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="108%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Category Name(EN) :</td>
        <td style="padding-right: 0px;"><input name="cat_name" type="text" class="textbox" id="cat_name" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('cat_name',$datalist); ?>"></td>
      </tr>
	  <tr>
        <td style="padding-left: 0px;">Category Name(AR) :</td>
        <td style="padding-right: 0px;"><input name="cat_name_ar" type="text" class="textbox" id="cat_name_ar" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('cat_name_ar',$datalist); ?>"></td>
      </tr>
	  <tr>
        <td style="padding-left: 0px;">Percentage :</td>
        <td style="padding-right: 0px;"><input name="percentage" type="text" class="textbox" id="percentage" onKeyPress="return isNumberKey(event);" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('percentage',$datalist); ?>"></td>
      </tr>
      <tr>
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onclick="location.href='account.php?page=agency-repair-category'">        </td>
        </tr>
...
<?php } ?>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center"> 
    <tr>
       ...
       <input name="sertxt" type="text" class="textbox" id="sertxt" style="width: 120px; font-weight: normal;" placeholder="Category name" value="<?php echo $_POST['sertxt']; ?>">
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="5%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onclick="check_all();" /></td>
       <td width="45%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Category Name</strong></td>
        <td width="17%" align="center" bgcolor="#333333"><strong>Percentage (%)</strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".AGENCYREPAIR." WHERE cat_name LIKE '%".$_POST["sertxt"]."%' ORDER BY id DESC LIMIT ".$startpoint.",".$perpage); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".AGENCYREPAIR." ORDER BY id DESC LIMIT ".$startpoint.",".$perpage);
	   }
  ...
  <tr>
   ...
   <tr>
  	<td align="right">
		...
		if(isset($_POST['search']))
		{
			echo Paging(AGENCYREPAIR,$perpage,"account.php?page=agency-repair-category&","cat_name LIKE '%".$_POST["sertxt"]."%' ORDER BY id DESC");
		
		}else
		{
			echo Paging(AGENCYREPAIR,$perpage,"account.php?page=agency-repair-category&");
		} ?>
	</td>
  </tr>
</table>
