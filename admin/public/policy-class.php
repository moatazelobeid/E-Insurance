...
if(isset($_POST['save']))
{
	...
	if($db->isExists('title',$_POST,POLICIES)){
		$errmsg = "- Policy Class already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,POLICIES,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Policy Class Created Sucessfully');location.href='account.php?page=policies';</script>";
	else if($result == 2)
		echo "<script>alert('Policy Class Creation Failed');location.href='account.php?page=policies';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,POLICIES) == 1){
	echo "<script>alert('Policy Class Updated Sucessfully');location.href='account.php?page=policies';</script>";
	}else
	echo "<script>alert('Policy Class Updation Failed');location.href='account.php?page=policies&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,POLICIES.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),POLICIES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=policies';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=policies';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=policies';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),POLICIES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=policies';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=policies';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=policies';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),POLICIES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=policies';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=policies';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=policies';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),POLICIES) == 1){
...
function confirmInput()
 {
  var retVal = confirm("Do you want to Delete these Policy Class ?");
   if( retVal == true ){
...
function validateManager()
{
	...
	if(!nameRegex.test(str.title.value.trim()))
	{
		str.title.style.borderColor = "RED";
		error = "- Enter Policy Class\n";
		flag = false;
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    <tr>
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 10px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Create/Manage Policy Class</strong></td>
      <td align="right" style="border-bottom: 1px solid #CCC; padding-bottom: 15px; padding-right: 0px;">
	  <a href="account.php?page=policies&view=policy-type" class="linkBtn <?php if($_GET['view'] == "policy-type") echo "active"; ?>">Manage Policy Type</a>
	  </td>
    </tr> 
</table>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
          <tr>
            <td align="left" style="border-bottom: 0px solid #99C; padding: 3px; background-color: #f2f2f2;"><strong>Add/Edit Policy Class </strong></td>
          </tr>
      </table>
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="108%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Policy Class(EN):</td>
        <td style="padding-right: 0px;"><input name="title" type="text" class="textbox" id="title" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('title',$datalist); ?>"></td>
      </tr>
	  <tr>
        <td style="padding-left: 0px;">Policy Class(AR):</td>
        <td style="padding-right: 0px;"><input name="title_ar" type="text" class="textbox" id="title_ar" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('title_ar',$datalist); ?>"></td>
      </tr>
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onclick="location.href='account.php?page=policies'">        </td>
        </tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 5px;">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="4%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onclick="check_all();" /></td>
       <td width="6%" align="center" bgcolor="#333333" style="padding-left: 5px;"><strong> Code </strong></td>
       <td width="15%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Policy Class </strong></td>
       <td width="40%" align="center" bgcolor="#333333" style="padding-left: 5px;"><strong>Policy Contents </strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".POLICIES." WHERE (title LIKE '%".$_POST["sertxt"]."%' OR title_ar LIKE '%".$_POST["sertxt"]."%') ORDER BY id ASC"); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".POLICIES." ORDER BY id ASC");
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
		...
		<td align="left"  style="padding-left: 5px;"><?php echo stripslashes($row["title"]); ?></td>
          <td align="center"  style="padding-left: 5px;">
          <?php $checkpolicy = mysql_query("select * from ".PRODUCTS." where policy_class_id ='".$row["id"]."' "); ?>
         <?php if(mysql_num_rows($checkpolicy) >0) {?><a href="account.php?page=policies&view=policy_details&policy_id=<?php echo $row["id"];?>&task=edit"><strong>Edit Details</strong></a>  <?php }else{ ?><a href="account.php?page=policies&view=policy_details&policy_id=<?php echo $row["id"];?> "><strong>Add Details</strong></a> <?php } ?></td>
		<td width="18%" align="center" >
		  <?php if($row['status'] == 0) echo "Inactive"; else echo "Active"; ?></td>
        <td width="17%" align="center" ><a href="account.php?page=policies&task=edit&id=<?php echo $row["id"]; ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit User Profile" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=policies&task=delete&id=<?php echo $row["id"];  ?> " onclick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;"></a></td>
  </tr>
        <?php }}else{
			?>
            <tr>
