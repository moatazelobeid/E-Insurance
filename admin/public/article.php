...
function check_single(checkid){
 ...
 {
  var retVal = confirm("Do you want to Delete these Article ?");
   if( retVal == true ){
	...
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
					$db->recordDelete(array("id" => $id),ARTICLETBL);
					$count++;
				
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=article&view=list';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=article&view=list';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=article&view=list';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),ARTICLETBL) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=article&view=list';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=article&view=list';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=article&view=list';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),ARTICLETBL) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Unpublished successfully');location.href='account.php?page=article&view=list';</script>";
			}else{
				echo "<script>alert('No Records Unpublished');location.href='account.php?page=article&view=list';</script>";
			}
		}else{
			echo "<script>alert('No Records Unpublished');location.href='account.php?page=article&view=list';</script>";
		}
...
if($_REQUEST['task']=='delete')
{
 ...
 $sqql="delete from ".ARTICLETBL." where id='".$_REQUEST['id']."'";
...
if($_GET['task']=='block'){
	...
	$sqlu=mysql_query("update ".ARTICLETBL." set status='0' where id='$id'");
if(mysql_affected_rows() > 0)
echo "<script>alert('Record Unpublished Sucessfully');location.href='account.php?page=article&view=list".$parturl."';</script>";
else
echo "<script>alert('Record Unpublished Failed');location.href='account.php?page=article&view=list".$parturl."';</script>";
...
if($_GET['task']=='active'){
	...
	$sqlp="update ".ARTICLETBL." set status='1' where id='$id'";

	$resultp=mysql_query($sqlp);
if(mysql_affected_rows() > 0)
echo "<script>alert('Record Published Sucessfully');location.href='account.php?page=article&view=list&part=".$_GET['part']."';</script>";
else
echo "<script>alert('Record Published Failed');location.href='account.php?page=article&view=list&part=".$_GET['part']."';</script>";
...

{
	...
	$btn_value = " Add New Article ";
	$cancel = "<a href='account.php?page=article&view=list'><u>Cancel</u></a>";
}
...

{
	...
	$cancel = "<a href='account.php?page=article&view=list'>Cancel</a>";
...

{
	...
	$allias=$_POST['title'];
	$allias=str_replace(array('?',' ','.'),array('','',''),$allias);
	$msg = "";
	
	$isdup=mysql_num_rows(mysql_query("select * from ".ARTICLETBL." where title='".$title."'"));
	if($isdup)
	{
		$msg = "- Article already exists";
		//header("location:account.php?page=article");
	}
	else
	{
		// save
		$sq_save = "INSERT INTO ".ARTICLETBL." (`title`, `allias`, `title_ar`, `smalltext`, `smalltext_ar`, `desc`, `desc_ar`, `created_date`, `modified_date`, `status`) VALUES ('".$title."', '".$allias."','".$title_ar."','".$smalltext."','".$smalltext_ar."','".$desc."','".$desc_ar."',now(),'0000-00-00 00:00:00','".$status."')";
		$res_save = mysql_query($sq_save);
		header("location:account.php?page=article&view=list");
	}
...

{
	...
	$sq_var = "SELECT * FROM ".ARTICLETBL." WHERE id = '".$_GET['id']."'";
...

{
	...
	$msg = "";
	$isdup=mysql_num_rows(mysql_query("select * from ".ARTICLETBL." where title='".$title."' and id!=".$page_id));
	if($isdup)
	{
		$msg = "- Article already exists";
	}
	...
	{
		// update
		 $sq_update ="UPDATE ".ARTICLETBL." SET `title` = '".$title."', `allias` = '".$allias."', `title_ar` = '".$title_ar."', `smalltext` = '".$smalltext."', `smalltext_ar` = '".$smalltext_ar."', `desc` = '".$desc."', `desc_ar` = '".$desc_ar."', `status` = '".$status."' WHERE id = ".$page_id;	
...
<div style="width: 100%; margin: 0 auto; margin-top: 10px;">
  <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
    <tr>
      <td width="67%" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Add/Edit Article</strong></td>
      <td width="33%" align="right" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-right: 0px; font-size: 14px; color: #036;"><a href="account.php?page=article&view=list"><div class="button_admin">View All Article</div></a></td>
    </tr>
 </table>
    <?php if($msg <> ""){
?>
...
<form action="" method="post" name="partcat_form" id="partcat_form" onSubmit="return masterValidate()">  <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
    <tr>
      <td colspan="2" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>List All Articles</strong></td>
      <td width="33%" align="right" style="border-bottom: 1px solid #99C; padding-bottom: 5px; padding-right: 0px; font-size: 14px; color: #036;"><a href="account.php?page=article"><div class="button_admin">Add New Article</div></a></td>
    </tr>
    <tr>
...
if(isset($_POST["search"])){
		  $j = 0;
		  $i = 0;
		 $sq = "SELECT * FROM ".ARTICLETBL." WHERE title LIKE '%".$_POST["stitle"]."%' ORDER BY id ASC LIMIT ".$startpoint.",".$perpage;
	  }	else
	  {			
				
		  $sq = "select * from ".ARTICLETBL." ORDER BY id LIMIT ".$startpoint.",".$perpage;}
          ...
          <tr <?php echo $bgcolor; ?>>
			...
			if($row["status"]=='0') { ?>
              <a href="account.php?page=article&task=active&view=list&id=<?php echo $row["id"]; ?>"><font color="#FF0000">InActive</font>
              <?php } else {?>
              </a><a href="account.php?page=article&task=block&view=list&id=<?php echo $row["id"]; ?>"><font color="#006633">Active</font>
                  
                  </a>
            <?php } ?></td>
			            <td align="center" ><?php echo date("d/m/Y",strtotime($row["created_date"])); ?></td>

            <td align="center" ><a href="public/article_view.php?id=<?php echo $row['id']; ?>" id="fancy"><img src="images/view.png"  width="16" height="16" border="0" title="View" style="cursor: pointer;" /></a>&nbsp;<a href="account.php?page=article&task=edit&id=<?php echo $row["id"];  ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=article&task=delete&view=list&id=<?php echo $row["id"];  ?>" onclick="return confirm('ARE YOU SURE TO DELETE!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete" style="cursor: pointer;"></a></td>
          </tr>
          <?php $i=$i+1;} ?>
          <tr>
      ...
      <input type="hidden" name="todo" id="todo" value=""/></td>
          ...
          <tr>
            <td colspan="8" align="center"><?php
			 ...
			 if(isset($_POST['search'])){
				  echo Paging(ARTICLETBL,$perpage,"account.php?page=article&view=list&","title LIKE '%".$_POST['stitle']."%' ORDER BY id DESC");
					
					}else{
					echo Paging(ARTICLETBL,$perpage,"account.php?page=article&view=list&");
					} 
