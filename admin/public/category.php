...
if(isset($_POST['save']))
{
	...
	if($db->isExists('cat_name',$_POST,CATEGORY)){
		$errmsg = "- Category title already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,CATEGORY,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Category Saved Sucessfully');location.href='account.php?page=category';</script>";
	else if($result == 2)
		echo "<script>alert('Category Saving Failed');location.href='account.php?page=category';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,CATEGORY) == 1){
	echo "<script>alert('Category Updated Sucessfully');location.href='account.php?page=category';</script>";
	}else
	echo "<script>alert('Category Updation Failed');location.href='account.php?page=category&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,CATEGORY.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),CATEGORY) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=category';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=category';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=category';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),CATEGORY) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=category';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=category';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=category';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),CATEGORY) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=category';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=category';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=category';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),CATEGORY) == 1){
...
setTimeout(fade_out, 2000);
 ...
 {
  var retVal = confirm("Do you want to Delete These Category ?");
   if( retVal == true ){
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    ...
    <tr>
      ...
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Create/Manage Category</strong></td>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
          <tr>
            <td align="left" style="border-bottom: 0px solid #99C; padding: 3px; background-color: #f2f2f2;"><strong>Add/Edit Category</strong></td>
          </tr>
        </table>
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="100%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Category(En):</td>
	  ...
	  <tr>
        <td style="padding-left: 0px;">Category(Ar):</td>
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onclick="location.href='account.php?page=category'">
        </td>
        </tr>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center"> 
    <tr>
       ...
       <input name="sertxt" type="text" class="textbox" id="sertxt" style="width: 120px; font-weight: normal;" value="<?php echo $_POST['sertxt']; ?>">
         <input type="submit" name="search" id="search" value=" Search " style="font-family: Verdana, Geneva, sans-serif; font-size: 11px;" />
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="6%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onclick="check_all();" /></td>
       <td width="56%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Category Name</strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".CATEGORY." WHERE cat_name LIKE '%".$_POST["sertxt"]."%' ORDER BY id DESC"); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".CATEGORY." ORDER BY id DESC");
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
        ...
        <td width="19%" align="center" ><a href="account.php?page=category&task=edit&id=<?php echo $row["id"];  ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit User Profile" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=category&task=delete&id=<?php echo $row["id"];  ?> " onclick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;"></a></td>
  </tr>
        <?php }}else{
			?>
            <tr>
        <td colspan="4" align="center" >No Category Listed</td>
		</tr>
            <?php
		}?>
 </table></td>
