...
if(isset($_POST['todo'])){
	...
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=sales-report';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=sales-report';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=sales-report';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=sales-report';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=sales-report';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=sales-report';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=sales-report';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=sales-report';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=sales-report';</script>";
		}
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 5px; margin-top: 3px;">
    <tr>
      ...
      <td align="right" style="border-bottom: 1px solid #CCC; padding-bottom: 12px; padding-left: 0px; font-size: 14px; color: #036;"><a href="account.php?page=sales-graph" class="linkBtn <?php if($_GET['view'] == "sales-graph") echo "active"; ?>">Graphical Report View</a></td>
    </tr> 
</table>
<?php if($msg <> ""){?>
    ...
    <tr>
      <?php $allrows=mysql_query("SELECT * FROM ".POLICYMASTER.",".POLICYPAYMENTS." where ".POLICYMASTER.".policy_no = ".POLICYPAYMENTS.".policy_no ORDER BY ".POLICYMASTER.".id DESC "); ?>
	   ...
	   <select name="policy_id" style="width: 100px; font-weight: normal; padding: 3px 3px 3px 3px;">
        ...
        <select name="renew_stat" style="width: 120px; font-weight: normal; padding: 3px 3px 3px 3px;">
		  <option value="">Renew Status</option>
		  <option value="1" <?php if($_POST["renew_stat"]=='1') echo 'selected="selected"';?>>New Policy</option>
          <option value="0" <?php if($_POST['renew_stat']=='0') echo 'selected="selected"';?>>Renew Policy</option>
        </select>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="3">
      <tr style="color: #FFF; background-color: #8B8585; height:22px">
       ...
       <td width="11%" style="padding-left: 5px;"><strong>Amount (SR)</strong></td>
	   ...
	   if(isset($_POST['search'])){
		   ...
		   if(isset($_POST['stat']) && $_POST['stat']!='')
			  {
			   ...
			   if(isset($_POST['renew_stat']) && $_POST['renew_stat']!='')
			  {
				  $renewid = $_POST['renew_stat'];
				  if($renewid =='0')
				  {
					   $whr[]=" renew_id='$renewid'";
					}else{
						$whr[]=" renew_id!='0'";
					}
		...
		$whr[]=" (registry_date BETWEEN '".$frm_date."' AND '".$to_date."')";
		  ...
		  $rs=  mysql_query("SELECT * FROM ".POLICYMASTER.",".POLICYPAYMENTS." ".$where."  ORDER BY ".POLICYMASTER.".id DESC");
	   }else{
      ...
      <tr <?php echo $bgcolor; ?> style="padding-bottom:5px;padding-top:5px;">
		...
		echo $transactionval['amount_paid'];
		?></td>
...
<td width="47%"><?php if(!empty($amountarr)){ 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 5px;">
  <tr>
	...
	<?php //echo Paging('ksa_policy_master,kas_policy_payments',$perpage,"account.php?page=sales-report&",$pwhr);?>
    <?php //}else{?>
    <?php //echo Paging('ksa_policy_master,kas_policy_payments',$perpage,"account.php?page=sales-report&");?>
    <?php //} ?>
    </td>
  </tr>
</table>
