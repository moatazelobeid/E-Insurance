...
if(isset($_POST['save']))
{
	...
	if($db->isExists('type_name',$_POST,DOCTYPES)){
		$errmsg = "- Document Type already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,DOCTYPES,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Document Type Created Sucessfully');location.href='account.php?page=doc-type';</script>";
	else if($result == 2)
		echo "<script>alert('Document Type Creation Failed');location.href='account.php?page=doc-type';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,DOCTYPES) == 1){
	echo "<script>alert('Document Type Updated Sucessfully');location.href='account.php?page=doc-type';</script>";
	}else
	echo "<script>alert('Document Type Updation Failed');location.href='account.php?page=doc-type&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,DOCTYPES.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),DOCTYPES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=doc-type';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=doc-type';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=doc-type';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),DOCTYPES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=doc-type';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=doc-type';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=doc-type';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),DOCTYPES) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=doc-type';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=doc-type';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=doc-type';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),DOCTYPES) == 1){
...
function confirmInput()
 {
  var retVal = confirm("Do you want to Delete these Document Type ?");
   if( retVal == true ){
...
function validateManager()
{
	...
	if(!nameRegex.test(str.type_name.value.trim()))
	{
		str.type_name.style.borderColor = "RED";
		error = "- Enter Document Type\n";
		flag = false;
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    ...
    <tr>
      ...
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Create/Manage Document Type </strong></td>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="380" valign="top" style="padding-right: 10px;">
    
    <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
          <tr>
            <td align="left" style="border-bottom: 0px solid #99C; padding: 3px; background-color: #f2f2f2;"><strong>Add/Edit Document Type </strong></td>
          </tr>
      </table>
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="100%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Document Type(EN):</td>
	  ...
	  <tr>
        <td style="padding-left: 0px;">Document Type(AR):</td>
      ...
      <tr>
        ...
        <td width="37%" style="padding-left: 0px;">Set Status:</td>
        
        <td width="63%" style="padding-right: 0px;"><select name="status" id="status" style="width: 193px; font-weight: normal;">
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onclick="location.href='account.php?page=doc-type'">        </td>
        </tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="6%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onclick="check_all();" /></td>
       ...
       <td width="48%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Document Type </strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".DOCTYPES." WHERE type_name LIKE '%".$_POST["sertxt"]."%' ORDER BY id ASC"); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".DOCTYPES." ORDER BY id ASC");
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
        ...
        <td width="19%" align="center" ><a href="account.php?page=doc-type&task=edit&id=<?php echo $row["id"]; ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit User Profile" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=doc-type&task=delete&id=<?php echo $row["id"];  ?> " onclick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;"></a></td>
  </tr>
        <?php }}else{
			?>
            <tr>
