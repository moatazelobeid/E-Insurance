...
if(isset($_POST['save']))
{
	...
	if($db->isExists('emp_type',$_POST,EMPTYPE)){
		$errmsg = "- Employee Type already exists";
	}else{
	// save record
	$result = $db->recordInsert($_POST,EMPTYPE,'');
	$recordId = mysql_insert_id();
	if($result == 1)
		echo "<script>alert('Employee Type Created Sucessfully');location.href='account.php?page=emp_type';</script>";
	else if($result == 2)
		echo "<script>alert('Employee Type Creation Failed');location.href='account.php?page=emp_type';</script>";
	}
}
if(isset($_POST['update']))
{
	unset($_POST['update']);
	// update record
	if($db->recordUpdate(array("id" => $id),$_POST,EMPTYPE) == 1){
	echo "<script>alert('Employee Type Updated Sucessfully');location.href='account.php?page=emp_type';</script>";
	}else
	echo "<script>alert('Employee Type Updation Failed');location.href='account.php?page=emp_type&task=edit&id=".$id."';</script>";
}
if($task == "edit" && $id != "")
{
	// get all data
	$datalist = $db->recordFetch($id,EMPTYPE.":".'id');
}
// delete all users
if(isset($_POST['todo'])){
	// case
	switch($_POST['todo']){
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordDelete(array('id' => $id),EMPTYPE) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Record Deleted Sucessfully');location.href='account.php?page=emp_type';</script>";
			}else{
				echo "<script>alert('No Records Deleted');location.href='account.php?page=emp_type';</script>";
			}
		}else{
			echo "<script>alert('No Records Found To Delete');location.href='account.php?page=emp_type';</script>";
		}
		break;
		case 'publishall':
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'1'),EMPTYPE) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records Published successfully');location.href='account.php?page=emp_type';</script>";
			}else{
				echo "<script>alert('No Records Published');location.href='account.php?page=emp_type';</script>";
			}
		}else{
			echo "<script>alert('No Records Published');location.href='account.php?page=emp_type';</script>";
		}
		...
		if(is_array($_POST['chkNo']) && count($_POST['chkNo']) > 0){
			...
			foreach($_POST['chkNo'] as $id){
				// delete user record
				if($db->recordUpdate(array("id" => $id),array('status'=>'0'),EMPTYPE) == 1){
					$count++;
				}
			}
			if($count > 0){
				echo "<script>alert('Records UnPublished successfully');location.href='account.php?page=emp_type';</script>";
			}else{
				echo "<script>alert('No Records UnPublished');location.href='account.php?page=emp_type';</script>";
			}
		}else{
			echo "<script>alert('No Records UnPublished');location.href='account.php?page=emp_type';</script>";
		}
...
{
	if($db->recordDelete(array('id' => $_GET['id']),EMPTYPE) == 1){
...
function confirmInput()
 {
  var retVal = confirm("Do you want to Delete these Employee Type ?");
   if( retVal == true ){
...
function validateManager()
{
	...
	{
		str.emp_type.style.borderColor = "RED";
		error = "- Enter Employee Type\n";
		flag = false;
		dataArray.push('emp_type');
	}
	else
	{
		str.emp_type.style.borderColor = "";
		flag = true;
		dataArray.pop();
	}
	if(flag == false)
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-bottom: 8px; margin-top: 10px;">
    ...
    <tr>
      ...
      <td align="left" style="border-bottom: 1px solid #CCC; padding-bottom: 5px; padding-left: 0px; font-size: 14px; color: #036;"><strong>Create/Manage Employee Type </strong></td>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" style="margin-bottom: 0px;">
          <tr>
            <td align="left" style="border-bottom: 0px solid #99C; padding: 3px; background-color: #f2f2f2;"><strong>Add/Edit Employee Type </strong></td>
          </tr>
        </table>
<form action="" method="post" name="s_fr" onSubmit="return validateManager();">
    ...
    <td style="padding-top: 5px;"><table width="108%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td style="padding-left: 0px;">Employee Type :</td>
        <td style="padding-right: 0px;"><input name="emp_type" type="text" class="textbox" id="emp_type" style="width: 187px; font-weight: normal;" value="<?php echo getElementVal('emp_type',$datalist); ?>"></td>
      </tr>
    ...
    ?>
          ...
          <?php } ?>
          <input type="button" name="cancel" id="cancel" value=" Reset " class="actionBtn" onclick="location.href='account.php?page=emp_type'">        </td>
        </tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
	  ...
	  <td width="6%" align="center" bgcolor="#333333"><input type="checkbox" name="chkAll" id="chkAll" onclick="check_all();" /></td>
       <td width="56%" align="left" bgcolor="#333333" style="padding-left: 5px;"><strong>Employee Type </strong></td>
	   ...
	   if(isset($_POST['search'])){
		  $rs=mysql_query("SELECT * FROM ".EMPTYPE." WHERE emp_type LIKE '%".$_POST["sertxt"]."%' ORDER BY id DESC"); 
	   }else{
		   $rs=mysql_query("SELECT * FROM ".EMPTYPE." ORDER BY id DESC");
	   }
      ...
      <tr <?php echo $bgcolor; ?>>
		...
		<td align="left"  style="padding-left: 5px;"><?php echo stripslashes($row["emp_type"]); ?></td>
		<td width="19%" align="center" >
		  <?php if($row['status'] == 0) echo "Inactive"; else echo "Active"; ?></td>
        <td width="19%" align="center" ><a href="account.php?page=emp_type&task=edit&id=<?php echo $row["id"];  ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0" title="Edit User Profile" style="cursor: pointer;"></a>&nbsp;<a href="account.php?page=emp_type&task=delete&id=<?php echo $row["id"];  ?> " onclick="return confirm('Are You Sure To Delete!');"><img src="images/delete.png" alt="Delete" width="16" height="16" border="0" title="Delete User" style="cursor: pointer;"></a></td>
  </tr>
        <?php }}else{
			?>
            <tr>
